...
export const createWishList = async pageLogin => {
    ...
    if (login === pageLogin) {
        const editBtn = createElement('button', {
            ...
            innerHTML: `
            <svg class="profile_edit-icon" width="38" height="38" viewBox="0 0 38 38" fill="none"
    ...
    if (user.description) {
        const description = createElement('p', {
            className: 'wishlist_description',
            ...
            for (const item of user.wish[title]) {
                ...
                if (login === pageLogin) {
                    const itemBtn = createElement('button', {
                        className: 'item_btn btn btn_castling',
                        textContent: 'Выбрать'
                    });
